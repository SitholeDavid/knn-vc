                        ...
                        for j, batch in progress_bar(enumerate(validation_loader), total=len(validation_loader), parent=mb):
                            ...
                            if j <= 4:
                                if steps == 0:
                                    ...
                                    wandb.log({'gt/y_{} step_{}'.format(j, steps): wandb.Audio(y[0].flatten().cpu().detach().numpy(), h.sampling_rate)})
                                    sw.add_figure('gt/y_spec_{}'.format(j), plot_spectrogram(x[0]), steps)

                                sw.add_audio('generated/y_hat_{}'.format(j), y_g_hat[0], steps, h.sampling_rate)
                                wandb.log({'generated/y_hat_{} step_{}'.format(j, steps): wandb.Audio( y_g_hat[0].flatten().cpu().detach().numpy(), h.sampling_rate)})
                                if USE_ALT_MELCALC:
